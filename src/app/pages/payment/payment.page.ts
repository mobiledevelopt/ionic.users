...
export class PaymentPage implements OnInit {
      ...
      (data: any) => {
        ...
        if (data && data.status === 200 && data.data) {
          ...
          if (info && info.length > 0) {
            ...
            const tf = info.filter((x) => x.id === "9");
            this.haveTF = tf && tf.length > 0 ? true : false;
            const qr = info.filter((x) => x.id === "10");
            ...
            this.util.showToast(
              this.util.getString("No Payment Found"),
              "danger",
              "bottom"
            );
          }
  ...
  proceed() {
    ...
    const url =
      this.api.baseUrl + "razorpay?" + this.api.JSON_to_URLEncoded(param);
    ...
    const param = {
      ...
      address:
        this.cart.deliveryAt === "home"
          ? JSON.stringify(this.cart.deliveryAddress)
          : "",
      driver_id: "",
  ...
  paypalPayment() {
    ...
    const url =
      this.api.baseUrl +
      "paypal/buyProduct?" +
      this.api.JSON_to_URLEncoded(param);
    const browser: any = this.iab.create(url, "_blank", options);
    browser.on("loadstop").subscribe((event) => {
      console.log("event?;>11", event);
      const navUrl = event.url;
  ...
  flutterpay() {
    ...
    console.log("to url===>", this.api.JSON_to_URLEncoded(param));
    const url =
      this.api.baseUrl + "flutterwave?" + this.api.JSON_to_URLEncoded(param);
    const browser: any = this.iab.create(url, "_blank", options);
    browser.on("loadstop").subscribe((event) => {
      console.log("event?;>11", event);
      const navUrl = event.url;
  ...
  paystackPay() {
    ...
    console.log("to url===>", this.api.JSON_to_URLEncoded(param));
    const url =
      this.api.baseUrl + "paystack?" + this.api.JSON_to_URLEncoded(param);
    const browser: any = this.iab.create(url, "_blank", options);
    browser.on("loadstop").subscribe((event) => {
      console.log("event?;>11", event);
      const navUrl = event.url;
  ...
  paytm() {
    ...
    console.log("to url===>", this.api.JSON_to_URLEncoded(param));
    const url =
      this.api.baseUrl + "paytm/pay?" + this.api.JSON_to_URLEncoded(param);
    const browser: any = this.iab.create(url, "_blank", options);
    browser.on("loadstop").subscribe((event) => {
      console.log("event?;>11", event);
      const navUrl = event.url;
  ...
  instaPay() {
    ...
    const param = {
      ...
      phone:
        this.util.userInfo && this.util.userInfo.mobile
          ? this.util.userInfo.mobile
          : "",
      send_email: "True",
    ...
    this.util.show();
    this.api
      .instaPay(url, param, this.instamojo.key, this.instamojo.token)
      .then(
        (data: any) => {
          console.log(data);
          this.util.hide();
