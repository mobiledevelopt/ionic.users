      ...
      if (element && element.discount === "0") {
        if (element.size === "1" || element.size === 1) {
          if (
            ...
            total =
              total +
              parseFloat(element.variations[0].items[element.variant].price) *
                element.quantiy;
          }
        } else {
    ...
    if (
      ...
      setTimeout(() => {
        ...
        if (this.freeShipping > this.totalPrice) {
          ...
          this.grandTotal =
            this.totalPrice - parseFloat(this.discount) + this.orderTax;
          this.grandTotal = parseFloat(this.grandTotal).toFixed(2);
          console.log("grand total===>>", this.grandTotal);
        }
      }, 1000);
      ...
      console.log(
        "extra delivery paarammmmm----<<<",
        this.userOrderTaxByStores
      );
      this.deliveryPrice = 0;
      this.discount =
        this.discount === null || this.discount === 0 || !this.discount
          ? 0
          : this.discount;
      this.grandTotal =
        this.totalPrice - parseFloat(this.discount) + parseFloat(this.orderTax);
      this.grandTotal = parseFloat(this.grandTotal).toFixed(2);
      console.log("grand total===>>", this.grandTotal);
    } else {
